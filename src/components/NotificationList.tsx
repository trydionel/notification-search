...
const TopicLink = ({ topic }) => {
  ...
  return (
    <a onClick={e => aha.drawer.showRecord(topic)}>
      {topic.referenceNum || `View ${topic.__typename}`}
    </a>
  )
}
...
const CommentNotificationRow = ({ notification, onRead, onStarred }) => {
  ...
  const onCreateTodo = async () => {
    const success = await createTodo(notification)
  ...
  return (
    <tr>
      ...
      <td width="256" style={{ verticalAlign: 'middle' }}>
      ...
      <td className="has-text-right" width="192" style={{ verticalAlign: 'middle' }}>
